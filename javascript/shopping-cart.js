  ...
  function createSizes() {
    var selectSize = document.getElementsByName("options-sizes");
      for (var i = 0; i < selectSize.length; i++) {
       createOptionsSizes(products[i].size, selectSize[i]);
      }
  }
    
  function createOptionsSizes(sizes, select) {   
   ...
   // ADD TO BASKET 
    ...
    function addClickEventToButtons() {
      ...
      for (var i = 0; i < button.length; i++) {
        var productId = button[i].parentElement.id;        
        button[i].addEventListener('click', createFunc(productId));  
      }
    }
